...
module.exports.config = {
  ...
  version: "1.0",
  category: "ai",
  description: "Get answers from the ai (ministral-small).",
  adminOnly: false,
  usePrefix: true,
  cooldown: 5,
};
...
module.exports.run = function ({ event, args }) {
  if (args.length < 1) { 
    return api.sendMessage(
      "Usage: Ministralv4 <question>\nExample: Ministralv4 what is life?",
      event.sender.id
    );
  }
  ...
  axios
    .get(`https://api.kenliejugarap.com/mistral-small-paid/?question=${encodeURIComponent(question)}`)
    .then((response) => {
