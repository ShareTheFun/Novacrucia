...
module.exports.config = {
  ...
  version: "1.0",
  category: "ai",
  description: "Get answers from the ai (ministral-3b).",
  adminOnly: false,
  usePrefix: true,
  cooldown: 5,
};
...
module.exports.run = function ({ event, args }) {
  if (args.length < 1) { 
    return api.sendMessage(
      "Usage: Ministral <question>\nExample: Ministral what is life?",
      event.sender.id
    );
  }
  ...
  axios
    .get(`https://api.kenliejugarap.com/ministral-3b-paid/?question=${encodeURIComponent(question)}`)
    .then((response) => {
